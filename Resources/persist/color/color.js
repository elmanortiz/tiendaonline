$(document).ready(function() {
    ...
    $(document).on('click', '#btnSave', function(event) {
        ...
        var id=$('#txtId');
        var name=$('#txtName');
        var table = $('#listadoColores').DataTable();
        ...
        if (errores==0) {
            ...
            $.ajax({
                data: data,
                url: Routing.generate('admin_registro_color'),
                type: 'POST',
                dataType: 'json',
    ...
    $(document).on('click', '#listadoColores>tbody>tr>td:nth-child(2)', function(event) {
        ...
        if (text=='TD' && id!=idForm) {
            $.ajax({
                url: Routing.generate('admin_recuperar_colores'),
                type: 'POST',
                data: {id: id},
    ...
    $(document).on('click', '.btDelete', function(event) {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    url: Routing.generate('admin_eliminar_colores'),
                    type: 'POST',
                    data: {param1: ids},
                    success:function(data){
                        if(data.error){

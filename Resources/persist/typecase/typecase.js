    $(document).ready(function() { 
        ...
        $(document).on("click","#btnSave", function() {   
            ...
            if($('#id').val() === '-1'){//Esto es cuando es edicion
                ...
                if (errores===0) {                                                                  
                    ...
                    var table = $('#typeCase').DataTable();
                    $.ajax({
                        type: 'POST',                    
                        data: {name:name, id:id},
                        url: Routing.generate('admin_typecase_insert'),
        ...
        $(document).on('click', '.sorting_1', function (event) {
            ...
            $.ajax({
                type: 'POST',                    
                data: {id:id},
                url: Routing.generate('admin_typecase_read'),
                success: function (data)
                {
            ...
            reverseButtons: true
                ...
                if (isConfirm) {
                    $.ajax({
                        type: 'POST',
                        data: {ids: ids},
                        url: Routing.generate('admin_typecase_delete'),
                        success: function (data)
                        {                            
                            $('.btnDelete').addClass('hidden');
                            $('.btnAdd').removeClass('hidden');

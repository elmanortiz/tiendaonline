...
//        $('.btAdd').click();
    ...
    $(document).on('click', '#btnSave', function(event) {
        ...
        if (errores==0) {
            ...
            $.ajax({
                data: data,
                url: Routing.generate('admin_registro_categoria'),
                type: 'POST',
                dataType: 'json',
    ...
    $(document).on('click', '#listadoCategoria>tbody>tr>td:nth-child(2)', function(event) {
        ...
        if (text=='TD' && id!=idForm) {
            $.ajax({
                url: Routing.generate('admin_recuperar_categorias'),
                type: 'POST',
                data: {id: id},
    ...
    $(document).on('click', '.btDelete', function(event) {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    url: Routing.generate('admin_eliminar_categorias'),
                    type: 'POST',
                    data: {param1: ids},
                    success:function(data){
                        if(data.error){

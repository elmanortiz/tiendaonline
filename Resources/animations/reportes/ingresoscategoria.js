...
//        $('#tabla').html('');
        ...
        if( fechaInicio != '' && fechaFin != '' ){
            $.ajax({
                url: Routing.generate('admin_ingresos_venta_categoria_filtro'),
                type: 'POST',
    ...
    $(document).on('click', '#exportar-pdf', function(event) {
        var fechaInicio = $("#fecha-inicio").val();
        var fechaFin = $("#fecha-fin").val();
        var url = Routing.generate('admin_ingresos_venta_categoria_pdf', {'fechaInicio': fechaInicio, 'fechaFin': fechaFin});
...
function drawTable(data) {
    ...
    row+= '<tr>';
    row+='<th style="text-align: center;" >Categoria de productos</th>';
...
function drawRow(rowData) {
    ...
    row+='<tr>';
    row+='<td style="text-align: left;">' + rowData.categoria + '</td>';
    ...
    row+='<td>';
    if(rowData.total != null && rowData.total !== null){
        row+=rowData.total; 
    } else {
        row+='0.00';
    }            
